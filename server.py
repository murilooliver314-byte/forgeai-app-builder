...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if route not in ('/api/generate','/api/modify','/api/seller-chat','/api/store','/api/orders'): return self.end_json({'error':'Rota não encontrada'},404)
        try:
            length=int(self.headers.get('Content-Length','0')); body=json.loads(self.rfile.read(length) or '{}')
            if route == '/api/orders':
                slug=str(body.get('slug','vendacertaai')).strip() or 'vendacertaai'; orders=body.get('orders',[])
            ...
            if route == '/api/store':
                slug=str(body.get('slug','vendacertaai')).strip() or 'vendacertaai'; products=body.get('products',[])
