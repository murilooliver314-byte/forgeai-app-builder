...
html.customer-request #home .actions{visibility:hidden}
html.customer-request.customer-ready #home .actions{visibility:visible}
html.customer-request:not(.customer-entered) main .view:not(#home){display:none!important}
html.customer-request:not(.customer-entered) header nav{display:none!important}
html.customer-request.customer-entered header nav{display:flex!important}
@media(max-width:700px){.customer-storefront header{padding:12px 16px}.customer-storefront header nav{overflow-x:auto;white-space:nowrap}.customer-storefront .customer-chat{margin-left:0}.customer-storefront header .logo{font-size:16px}.customer-storefront .customer-tab{padding:9px 11px}}</style>'''
    page=page.replace('</head>',gallery_css+"<script>(function(){var q=new URLSearchParams(location.search);if(q.has('loja')&&( !q.has('cliente')||q.get('cliente')==='1'))document.documentElement.classList.add('customer-request')})();</script></head>")
...
// Dedicated customer storefront. Auth is resolved before customer mode is enabled.
(function customerStorefront(){
  const params=new URLSearchParams(window.location.search);
  const requested=params.has('loja') && (!params.has('cliente') || params.get('cliente')==='1');
  if(!requested)return;
  // This runs after app.js, but is also safe if it is evaluated before body exists.
  document.documentElement.classList.add('customer-request');
  let customerMode=false, customerStarted=false, authResolved=false;
  const el=id=>document.getElementById(id);
  function active(){return requested&&authResolved&&!window.authUser && !authUser;}
  function setHome(){
    customerStarted=false; document.documentElement.classList.remove('customer-entered');
    const nav=document.querySelector('header nav'); if(nav)nav.style.display='none';
    const back=el('backBtn'); if(back)back.style.display='none';
    if(typeof view==='function')view('home',false); window.scrollTo(0,0);
  }
  function customerNav(){
    const nav=document.querySelector('header nav'); if(!nav)return;
    nav.innerHTML='<button class="customer-tab active" data-customer-view="store">Produtos</button><button class="customer-tab" data-customer-view="videos">Vídeos</button><button class="customer-chat" data-customer-view="assistant">✦ Falar com a vendedora</button>';
    nav.style.display='flex';
  }
  function markTab(id){document.querySelectorAll('[data-customer-view]').forEach(x=>x.classList.toggle('active',x.dataset.customerView===id));}
  function enterCustomer(target){
    if(!active())return;
    customerStarted=true; document.documentElement.classList.add('customer-entered'); customerNav();
    const back=el('backBtn'); if(back)back.style.display='inline-flex';
    ...
    document.querySelector('#store .section-head .actions')?.remove(); document.querySelector('#videos .section-head .actions')?.remove();
    const id=target||'store'; if(typeof view==='function')view(id); markTab(id);
    if(id==='store'&&typeof loadProducts==='function')loadProducts();
    if(typeof loadVideos==='function')loadVideos();
  }
  function setup(){
    if(!active()||customerMode)return; customerMode=true;
    document.body.classList.add('customer-storefront'); document.documentElement.classList.add('customer-ready');
    document.querySelector('.beta-banner')?.remove();
    ['#headerLogin','#accountBadge','#themeBtn','#trialBtn','#loginBtn','#trialHelp','#planAccessBtn','#admBtn','#videoAdmin','#foundationPanel'].forEach(s=>document.querySelector(s)?.remove());
    ...
    if(home)home.innerHTML='<button type="button" class="primary" id="customerEnterBtn">Ver produtos da loja</button><button type="button" class="outline" id="customerChatBtn">Falar com a vendedora</button><small class="action-help">Escolha um produto ou converse com a vendedora da loja.</small>';
    setHome();
  }
  // Delegation avoids the setup/listener race and also survives rerenders.
  document.addEventListener('click',function(e){
    const enter=e.target.closest?.('#customerEnterBtn'); if(enter){e.preventDefault();enterCustomer('store');return}
    const chat=e.target.closest?.('#customerChatBtn'); if(chat){e.preventDefault();enterCustomer('assistant');return}
    const tab=e.target.closest?.('[data-customer-view]'); if(tab&&active()){e.preventDefault();enterCustomer(tab.dataset.customerView);return}
    const back=e.target.closest?.('#backBtn'); if(back&&active()&&customerStarted){e.preventDefault();setHome();}
    const logo=e.target.closest?.('.logo'); if(logo&&active()&&customerStarted){e.preventDefault();setHome();}
  },true);
  // The auth check is the gate: a logged-in entrepreneur never gets customer DOM.
  fetch('/api/auth/me',{credentials:'same-origin'}).then(r=>r.json()).then(d=>{authResolved=true;if(!d.ok&&!authUser)setup();}).catch(()=>{authResolved=true;if(!authUser)setup();});
})();'''
