...
def store_access_active(slug):
    ...
    return True if slug=='vendacertaai' and not user else bool(user and (access_status(user)['trial_active'] or access_status(user)['plan_active']))
...
def vendacerta_page():
    ...
    # Keep the public catalog visible even before client-side hydration completes.
    fallback=''.join('<article class="product"><div class="product-art"></div><div class="product-info"><strong>'+str(p.get('name','Produto'))+'</strong><p>'+str(p.get('desc','Produto disponível na loja.'))+'</p><div class="price">'+str(p.get('price',''))+'</div><button class="primary" type="button">Comprar via Pix rápido</button></div></article>' for p in store_products('vendacertaai'))
    page=page.replace('<div class="products" id="products"></div>','<div class="products" id="products">'+fallback+'</div>')
...
@media(max-width:700px){.customer-storefront header{padding:12px 16px}.customer-storefront header nav{overflow-x:auto;white-space:nowrap}.customer-storefront .customer-chat{margin-left:0}.customer-storefront header .logo{font-size:16px}.customer-storefront .customer-tab{padding:9px 11px}}</style>'''
    page=page.replace('</head>','<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Ctext y=%22.9em%22 font-size=%2290%22%3EV%3C/text%3E%3C/svg%3E"><link rel="apple-touch-icon" href="data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Ctext y=%22.9em%22 font-size=%2290%22%3EV%3C/text%3E%3C/svg%3E">'+gallery_css+"<script>(function(){var q=new URLSearchParams(location.search);document.documentElement.classList.add('vc-auth-loading');if(q.has('loja')&&( !q.has('cliente')||q.get('cliente')==='1'))document.documentElement.classList.add('customer-request')})();</script></head>")
...
(function authPanelBootstrap(){fetch('/api/auth/me',{credentials:'same-origin',cache:'no-store'}).then(r=>r.json()).then(d=>{if(d.ok&&d.user){storeSlug=d.user.slug;setAuth(d.user);document.documentElement.classList.remove('customer-request','vc-anonymous','vc-auth-loading');window.history.replaceState({},'',window.location.pathname);if(typeof view==='function')view('productsView',false)}}).catch(()=>{});})(); // Dedicated customer storefront. Resolve auth before enabling public mode.
...
(function customerStorefront(){
  ...
  fetch('/api/auth/me',{credentials:'same-origin',cache:'no-store'}).then(r=>r.json()).then(d=>{
    authResolved=true;
    if(!d.ok&&!authUser)enterCustomer();else document.documentElement.classList.remove('customer-request');
  }).catch(()=>{authResolved=true;if(!authUser)enterCustomer();});
})();'''
    page=page.replace('<script src="app.js"></script>','<script>'+files['app.js'].replace('</script>','<\\/script>')+extra_js+'</script><script>(function(){function q(s){return document.querySelector(s)}function show(k){var m=q("#modal"),b=q("#modalBody");if(!m||!b)return;b.innerHTML=k==="signup"?`<small>TESTE GRÁTIS</small><h2>Comece em 48 horas</h2><input id="rName" placeholder="Seu nome"><input id="rBusiness" placeholder="Nome do negócio"><input id="rEmail" placeholder="Seu e-mail" type="email"><input id="rPassword" placeholder="Crie uma senha (mín. 6 caracteres)" type="password"><button class="primary" id="rSubmit">Criar minha conta</button>`:`<small>ENTRAR</small><h2>Acesse sua conta</h2><input id="rEmail" placeholder="Seu e-mail" type="email"><input id="rPassword" placeholder="Senha" type="password"><button class="primary" id="rSubmit">Entrar</button>`;m.classList.add("open");q("#rSubmit").onclick=async function(){var p=k==="signup"?{name:q("#rName").value,business:q("#rBusiness").value,email:q("#rEmail").value,password:q("#rPassword").value}:{email:q("#rEmail").value,password:q("#rPassword").value};try{var r=await fetch(k==="signup"?"/api/auth/signup":"/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)}),d=await r.json();if(!r.ok)throw new Error(d.error||"Não foi possível concluir.");window.location.assign("/vendacertaai")}catch(e){alert(e.message)}}}document.addEventListener("click",function(e){if(e.target.closest("#trialBtn"))show("signup");if(e.target.closest("#loginBtn"))show("login")})})();</script>')
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ('/vendacertaai','/vendacertaai/'):
            raw=vendacerta_page(); user=session_user(self); query=parse_qs(urlparse(self.path).query); owner_page=bool(user) and not (bool(query.get('loja')) and query.get('cliente',[''])[0] in ('','1')); raw=raw.replace(b"document.documentElement.classList.add('vc-auth-loading');",b"document.documentElement.classList.add('vc-authenticated');") if owner_page else raw; raw=raw.replace(b'<section id="home" class="view active">',b'<section id="home" class="view">').replace(b'<section id="productsView" class="view">',b'<section id="productsView" class="view active">') if owner_page else raw; boot_user=json.dumps({k:user.get(k) for k in ('id','name','business','slug','trial_ends_at')} if user else {},ensure_ascii=False).replace('<','\u003c'); raw=raw.replace(b'</body>',('<script>setTimeout(function(){try{var u='+boot_user+';if(typeof enterStore==="function")enterStore(u,"Sessão recuperada")}catch(e){}},0)</script></body>').encode('utf-8')) if user else raw; self.send_response(200); self.send_header('Content-Type','text/html; charset=utf-8'); self.send_header('Content-Length',str(len(raw))); self.end_headers(); self.wfile.write(raw); return
        if path=='/api/health':
