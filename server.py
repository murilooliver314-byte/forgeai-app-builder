...
def gemini_seller(message, history=None):
    key = os.environ.get('GEMINI_API_KEY', '').strip()
    if not key:
        return None
    history = history or []
    catalog = 'Produto especial — R$ 49,90; Mais vendido — R$ 79,90; Novidade — R$ 99,90.'
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if route not in ('/api/generate','/api/modify','/api/seller-chat'): return self.end_json({'error':'Rota não encontrada'},404)
        try:
            length=int(self.headers.get('Content-Length','0')); body=json.loads(self.rfile.read(length) or '{}')
            if route == '/api/seller-chat':
                message=str(body.get('message','')).strip(); history=body.get('history',[])
                if not message: return self.end_json({'error':'Digite uma mensagem'},400)
                reply=gemini_seller(message, history if isinstance(history,list) else [])
