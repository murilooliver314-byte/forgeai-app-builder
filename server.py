...
// Dedicated customer storefront: shared links use cliente=1 and never expose entrepreneur controls.
(function customerStorefront(){
  ...
  function setup(){if(!active())return; document.body.classList.add('customer-storefront');
    document.querySelector('.beta-banner')?.remove(); document.querySelector('#headerLogin')?.remove(); document.querySelector('#accountBadge')?.remove(); document.querySelector('#themeBtn')?.remove();
    ['#trialBtn','#loginBtn','#trialHelp','#planAccessBtn','#admBtn','#videoAdmin','#foundationPanel'].forEach(s=>document.querySelector(s)?.remove());
    document.querySelectorAll('header nav button').forEach(b=>b.remove());
    const nav=document.querySelector('header nav'); if(nav){nav.innerHTML='<button class="customer-tab active" data-customer-view="store">Produtos</button><button class="customer-tab" data-customer-view="videos">Vídeos</button><button class="customer-chat" data-customer-view="assistant">✦ Falar com a vendedora</button>';nav.querySelectorAll('[data-customer-view]').forEach(b=>b.onclick=()=>{const id=b.dataset.customerView;view(id);nav.querySelectorAll('.customer-tab').forEach(x=>x.classList.toggle('active',x===b));});}
    const title=document.querySelector('#store h2');if(title)title.textContent='Produtos da loja';const copy=document.querySelector('#store p');if(copy)copy.textContent='Escolha um produto ou fale com a vendedora.';
    document.querySelector('#store .section-head .actions')?.remove(); document.querySelector('#videos .section-head .actions')?.remove();
    document.querySelector('#store')?.classList.add('customer-default'); view('store',false); loadProducts(); loadVideos();
  }
  const previousSetAuth=setAuth; setAuth=function(user){previousSetAuth(user);if(!user)setup();};
  // Wait for the existing session check before changing the shell, so an authenticated
  // entrepreneur opening a shared URL keeps the normal management experience.
