...
def gemini_seller(message, history=None, catalog=None):
    key = os.environ.get('GEMINI_API_KEY', '').strip()
    if not key:
        return None
    history = history or []
    ...
    prompt = '''Você é a vendedora virtual da VendaCertaAI. Responda em português brasileiro, de forma simpática, objetiva e persuasiva, somente por texto. Use o catálogo abaixo para recomendar produtos e preços. Nunca invente produtos, preços, descontos, estoque ou prazo de entrega. Se o cliente quiser comprar, peça nome, produto e quantidade e diga que o pedido será encaminhado. Se não souber algo, diga que precisa confirmar com o vendedor.
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if route not in ('/api/generate','/api/modify','/api/seller-chat','/api/store','/api/orders','/api/order/create','/api/auth/signup','/api/auth/login','/api/auth/logout'): return self.end_json({'error':'Rota não encontrada'},404)
        try:
            ...
            if route == '/api/auth/signup':
                ...
                salt,digest=password_hash(password); now=time.time(); user={'id':secrets.token_hex(12),'email':email,'name':name,'business':business,'slug':slug,'salt':salt,'password_hash':digest,'trial_ends_at':now+48*3600}
                data['users'][email]=user; token=secrets.token_urlsafe(32); data['sessions'][token]=email; save_auth(data); save_store_products(slug,DEFAULT_PRODUCTS.copy()); save_store_orders(slug,[])
            ...
            if route == '/api/order/create':
                slug=str(body.get('slug','vendacertaai')).strip() or 'vendacertaai'; order=body.get('order',{})
                if not isinstance(order,dict) or not str(order.get('product','')).strip(): return self.end_json({'error':'Pedido inválido.'},400)
                orders=store_orders(slug); orders.append({'id':str(order.get('id') or str(time.time()).replace('.','')[-8:]),'product':str(order.get('product')),'price':str(order.get('price','')),'status':'Novo'})
            ...
            if route == '/api/seller-chat':
                message=str(body.get('message','')).strip(); history=body.get('history',[]); catalog=body.get('catalog',[])
                if not message: return self.end_json({'error':'Digite uma mensagem'},400)
                reply=gemini_seller(message, history if isinstance(history,list) else [], catalog)
                if reply: return self.end_json({'ok':True,'reply':reply,'ai_enabled':True})
