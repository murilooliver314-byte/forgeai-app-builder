...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if route not in ('/api/generate','/api/modify','/api/seller-chat','/api/store','/api/orders','/api/order/create','/api/payment/create','/api/customer/profile','/api/agent/settings','/api/auth/signup','/api/auth/login','/api/auth/logout'): return self.end_json({'error':'Rota não encontrada'},404)
        try:
            ...
            if route == '/api/payment/create':
                slug=str(body.get('slug','vendacertaai')).strip() or 'vendacertaai'; order=body.get('order',{}); session_id=str(body.get('session_id','')).strip()[:100]; customer=body.get('customer',{}) if isinstance(body.get('customer',{}),dict) else {}
                ...
                order_id=str(order.get('id') or str(time.time()).replace('.','')[-8:]); preference,error=create_mp_preference(order.get('product'),amount,order_id,slug)
                ...
                orders=store_orders(slug); saved={'id':order_id,'product':str(order.get('product')),'price':str(order.get('price','')),'customer_name':str(customer.get('name','')).strip()[:100],'customer_phone':str(customer.get('phone','')).strip()[:30],'session_id':session_id,'status':'Pagamento pendente','payment_preference_id':preference.get('id'),'checkout_url':preference.get('sandbox_init_point') or preference.get('init_point')}; orders.append(saved); save_store_orders(slug,orders)
