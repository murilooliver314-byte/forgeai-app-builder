...
def apply_payment_update(payment):
    ...
    for order in orders:
        if str(order.get('id'))==order_id:
            order['status']=payment_label(payment.get('status')); order['payment_id']=str(payment.get('id','')); order['payment_status']=str(payment.get('status','')); order['updated_at']=time.time(); changed=True
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if route not in ('/api/generate','/api/modify','/api/seller-chat','/api/store','/api/orders','/api/order/create','/api/payment/create','/api/payment/webhook','/api/customer/profile','/api/agent/settings','/api/auth/signup','/api/auth/login','/api/auth/logout'): return self.end_json({'error':'Rota não encontrada'},404)
        try:
