...
// Dedicated customer storefront: customer shared links begin on a safe landing screen.
(function customerStorefront(){
  const params=new URLSearchParams(window.location.search), requested=params.has('loja') && (!params.has('cliente') || params.get('cliente')==='1');
  let customerMode=false, customerStarted=false;
  function active(){return requested&&!authUser}
  function customerNav(){const nav=document.querySelector('header nav');if(!nav)return;nav.innerHTML='<button class="customer-tab active" data-customer-view="store">Produtos</button><button class="customer-tab" data-customer-view="videos">Vídeos</button><button class="customer-chat" data-customer-view="assistant">✦ Falar com a vendedora</button>';nav.querySelectorAll('[data-customer-view]').forEach(b=>b.onclick=()=>{const id=b.dataset.customerView;customerStarted=true;view(id);nav.querySelectorAll('.customer-tab').forEach(x=>x.classList.toggle('active',x===b));});nav.style.display='flex'}
  function enterCustomer(){if(!active())return;customerStarted=true;customerNav();$('backBtn').style.display='inline-flex';const title=document.querySelector('#store h2');if(title)title.textContent='Produtos da loja';const copy=document.querySelector('#store p');if(copy)copy.textContent='Escolha um produto ou fale com a vendedora.';document.querySelector('#store .section-head .actions')?.remove();document.querySelector('#videos .section-head .actions')?.remove();view('store');loadProducts();loadVideos()}
  function setup(){if(!active()||customerMode)return;customerMode=true;document.body.classList.add('customer-storefront');document.querySelector('.beta-banner')?.remove();document.querySelector('#headerLogin')?.remove();document.querySelector('#accountBadge')?.remove();document.querySelector('#themeBtn')?.remove();['#trialBtn','#loginBtn','#trialHelp','#planAccessBtn','#admBtn','#videoAdmin','#foundationPanel'].forEach(s=>document.querySelector(s)?.remove());const nav=document.querySelector('header nav');if(nav){nav.innerHTML='';nav.style.display='none'}const home=document.querySelector('#home .actions');if(home)home.innerHTML='<button class="primary" id="customerEnterBtn">Ver produtos da loja</button><button class="outline" id="customerChatBtn">Falar com a vendedora</button><small class="action-help">Escolha um produto ou converse com a vendedora da loja.</small>';$('customerEnterBtn')?.addEventListener('click',enterCustomer);$('customerChatBtn')?.addEventListener('click',()=>{enterCustomer();view('assistant');document.querySelectorAll('[data-customer-view]').forEach(x=>x.classList.toggle('active',x.dataset.customerView==='assistant'))});$('backBtn').style.display='none';$('backBtn').onclick=()=>{customerStarted=false;document.querySelector('header nav').style.display='none';view('home',false);window.scrollTo(0,0)};document.querySelector('.logo')?.addEventListener('click',()=>{if(customerStarted){customerStarted=false;document.querySelector('header nav').style.display='none';view('home',false);$('backBtn').style.display='none'}});view('home',false)}
  fetch('/api/auth/me').then(r=>r.json()).then(d=>{if(!d.ok&&!authUser)setup()}).catch(()=>{if(!authUser)setup()});
})();'''
