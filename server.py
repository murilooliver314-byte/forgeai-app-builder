...
(function authPanelBootstrap(){fetch('/api/auth/me',{credentials:'same-origin',cache:'no-store'}).then(r=>r.json()).then(d=>{if(d.ok&&d.user){storeSlug=d.user.slug;setAuth(d.user);document.documentElement.classList.remove('customer-request','vc-anonymous','vc-auth-loading');window.history.replaceState({},'',window.location.pathname);if(typeof view==='function')view('home',false)}}).catch(()=>{});})(); // Dedicated customer storefront. Resolve auth before enabling public mode.
